...
def save_to_excel(results, input_format, output_filepath, VID):
...
def adjust_cells(output_file_path, width=15):
    # Load the workbook
...
def main(input_filepath, input_format, output_filepath, vid_list, locn_list):
    ...
    for unit in vid_list:
        ...
        save_to_excel(results, input_format, output_filepath, unit)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", type=str, required=True, help="Output folder path where the output goes")
    parser.add_argument("--aquareport", type=str, required=True, help="Input file name (include .csv)")
    ...
    input_filepath = Path('/'.join([directory, args.aquareport]))
    output_filepath = Path('/'.join([directory, args.outputfile]))
    input_format = Path('/'.join([directory, args.format]))
    ...
    if not (input_filepath.exists() and input_format.exists()) :
        ...
        main(input_filepath, input_format, output_filepath, vid_list, locn_list)